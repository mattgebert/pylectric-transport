...
FILE_DESCRIPTOR1 = "Devs4_04_run10_V01-V02_60.00K-220.00K 26.0V-66.0V.txt" # Bare Graphene
FILE_DESCRIPTOR2 = "Devs4_04_run10_V08-V07_60.00K-220.00K 26.0V-66.0V.txt" # Gallium Oxide

##Create new folder for analysis.
saveLoc = os.getcwd() + PRELUDE + RAW_DATA_DIR + "\\"
i = 0
saveTarget = saveLoc + "analysis" + str(i) + "\\"
...
# rvt_obj2 = Meas_Temp_GatedResistance(temps = temps2, vg=vgs2, resistivity=data1_reshaped)
########################################################################################################
####                            Manually remove LA Phonon Contrib.                                 #####
########################################################################################################
# Da = 27.8
Da = 22.2
# Da = 18.0
data1_LA_phonons = graphene.Graphene_Phonons.rho_LA(np.tile(temps1, (len(vgs1),1)), Da=Da).transpose()
data2_LA_phonons = graphene.Graphene_Phonons.rho_LA(np.tile(temps2, (len(vgs2),1)), Da=Da).transpose()
data1_LA_phonons.shape

data1_onlyExp = data1_reshaped - data1_LA_phonons
data2_onlyExp = data2_reshaped - data2_LA_phonons

rvt_obj1 = Meas_Temp_GatedResistance(temps = temps1, vg=vgs1, resistivity=data1_onlyExp)
rvt_obj2 = Meas_Temp_GatedResistance(temps = temps2, vg=vgs2, resistivity=data2_onlyExp)
...
rvt_obj1.plot_Rho_vT()

## Conduct fits
#Bare graphene sample
obj1_params_exp_sio2, obj1_covar_exp_sio2 = graphene.Graphene_Phonons.fit_ROP_Gr_on_Sio2(rvt_obj1)
obj1_params_exp_generic, obj1_covar_exp_generic = graphene.Graphene_Phonons.fit_ROP_Gr_on_Dielectric(rvt_obj1)
...
exp_ax.set_yscale("linear")

# PARAMETERS:
# param_headers = ["$B_1$"]
param_headers = [r"$\alpha$","$B_1$","$E_0$"]

param_rows= []
param_rows.append("SiO$_2$/Gr: SiO$_2$ Fit")
# param_rows.append("SiO$_2$/Gr/Ga$_2$O$_3$: SiO$_2$ Fit")
param_rows.append("SiO$_2$/Gr: Generic single mode fit")
...
param_list.append(tuple(["%0.03f $\pm$ %0.03f" % (obj1_params_exp_sio2[i], math.sqrt(obj1_covar_exp_sio2[i,i])) for i in range(2)] + ["-"]))
# param_list.append(tuple(["%0.03f $\pm$ %0.03f" % (obj2_params_exp_sio2[i], math.sqrt(obj2_covar_exp_sio2[i,i])) for i in range(2)] + ["-"]))
param_list.append(tuple(["%0.03f $\pm$ %0.03f" % (obj1_params_exp_generic[i], math.sqrt(obj1_covar_exp_generic[i,i])) for i in range(3)]))
# param_list.append(tuple(["%0.03f $\pm$ %0.03f" % (obj2_params_exp_ga2o3[i], math.sqrt(obj2_covar_exp_ga2o3[i,i])) for i in range(2)] + ["-"]))
# param_list.append(tuple(["%0.03f $\pm$ %0.03f" % (obj2_params_exp_generic[i], math.sqrt(obj2_covar_exp_generic[i,i])) for i in range(3)]))

exp_ax.set_title(r"Devs4_04 Run10; Bare graphene & Ga$_2$O$_3$ covered graphene" + "\n"
 + "Linear acoustic phonons subracted with $D_A=$" + str(Da))
plt.table(cellText=param_list,
        ...
        bbox=(1.575,0.6,0.8,0.4))
saveTarget
graphene.Graphene_Phonons.Chen_Gr_ROP_SiO2(([250.0],[26.0]))
saveStr = "Phonons %0.02f" % temps1[0] + "K-%0.02f" % temps1[-1] + "K %0.01fV" % vgs1[0] + "-%0.01fV" % vgs1[-1]
saveLoc = saveTarget + saveStr + ".png"
integer = 0
if os.path.exists(saveLoc):
    saveLoc = saveTarget + saveStr + str(integer) + ".png"
