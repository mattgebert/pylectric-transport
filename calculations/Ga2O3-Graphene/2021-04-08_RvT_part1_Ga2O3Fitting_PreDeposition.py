...
GEO_FACTOR = (408.9/256.75) #Geometry factor of the device - what's the conversion from resistance to resistivity? (\rho = R * (Geo_factor) = R * (W / L))
...
# vgs_exp = np.linspace(22,58,10) #6 voltages, 5-30 includive.
# vgs_exp = np.linspace(26,58,9) #6 voltages, 5-30 includive.
vgs_exp = np.linspace(30,58,8) #6 voltages, 5-30 includive.
fit_dirac_exp = fitted_dirac_points[t1:t2]
temps_exp = temps[t1:t2]
temps_exp
exp_meas_objs = meas_objs[t1:t2]
...
# # vgs_exp = np.linspace(34,66,9) #6 voltages, 5-30 includive.
# # vgs_exp = np.linspace(34,70,10) #6 voltages, 5-30 includive.
# vgs_exp = np.linspace(22,70,13) #6 voltages, 5-30 includive.
# fit_dirac_exp = fitted_dirac_points[t1:t2]
# temps_exp = temps[t1:t2]
# exp_meas_objs = meas_objs[t1:t2]
...
exp_1D_data
np.savetxt(target2 + FILE_DESCRIPTOR + "_RhoVG-Extraction_Exp %0.02f" % temps_exp[0] + "K-%0.02f" % temps_exp[-1] + "K %0.01fV" % vgs_exp[0] + "-%0.01fV" % vgs_exp[-1] + ".txt", exp_1D_data, delimiter=",", header="Temperature (K),Gate Voltage (V),Resisitivity (Ohms)")
...
params_exp, covar_exp = graphene.Graphene_Phonons.fit_ROP_Gr_between_Sio2_Ga2O3(rvt_obj)
params_exp_sio2, covar_exp_sio2 = graphene.Graphene_Phonons.fit_ROP_Gr_on_Sio2(rvt_obj)
# params_exp
# len(params_exp)
# len(vgs_exp)
...
offsets_exp_sio2 = [params_exp_sio2[-(len(vgs_exp)) + i] + i * sep for i in range(len(vgs_exp))]
...
kwargs = {"s":1}
exp_ax = rvt_obj.plot_Rho_vT(c = colourCycle + colourCycle, labels=[str(vg) + " V" for vg in vgs_exp], offsets=offsets_exp_sio2, **kwargs)
# Plot Fits
rvt_obj.global_RTVg_plot(function=graphene.Graphene_Phonons.rho_ROP_Gr_between_SiO2_Ga2O3, params=tuple(params_exp), ax=exp_ax, c=colourCycle + colourCycle, points=30, linewidth=0.3, style="-", singleLabel="Ga2O3 Fit", offsets=offsets_exp, T_max=300)
rvt_obj.global_RTVg_plot(function=graphene.Graphene_Phonons.rho_ROP_Gr_on_SiO2, params=tuple(params_exp_sio2), ax=exp_ax, c=colourCycle + colourCycle, points=30, linewidth=0.3, style="-.", singleLabel="SiO2 Fit", offsets=offsets_exp_sio2, T_max=300)
# LEGEND
...
# PARAMETERS:
param_headers = ["$B_1$"]
# param_rows = ["Ga2O3/SiO2 Fit"]
param_rows = ["SiO2 Fit", "Ga2O3/SiO2 Fit"]
param_list = []
param_list.append(tuple(["%0.03f $\pm$ %0.03f" % (params_exp_sio2[i], math.sqrt(covar_exp_sio2[i,i])) for i in range(1)]))
