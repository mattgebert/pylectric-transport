...
target2 = os.getcwd() + RAW_DATA_DIR + "\\" + FILE_DESCRIPTOR + "\\" + "03 Phonon Analysis (Ave & Fitted)" + "\\"
...
for meas_set_i in range(len(meas_objs)):
    ...
    for meas_obj_i in range(len(meas_set)):
        ...
        try:
            ...
            # params, covar = graphene.Graphene_Gated.min_cond_quadratic_fit(meas_obj)
...
# Data
lin_ax = rvt_obj_linear.plot_Rho_vT(c = colourCycle, labels=["" + str(vg) + " V" for vg in vgs_linear], offsets=offsets)
# Fits
rvt_obj_linear.global_RTVg_plot(function=graphene.Graphene_Phonons.rho_Graphene_LA, params=tuple(params_lin), ax=lin_ax, c=colourCycle2, linewidth=0.3, singleLabel="fit LA", style="-.", offsets=offsets)
...
plt.savefig(target2 + "01 Phonons LA %0.02f" % temps_linear[0] + "K-%0.02f" % temps_linear[-1] + "K.png", bbox_inches="tight")
...
exp_ax = rvt_obj.plot_Rho_vT(c = colourCycle2, labels=[str(vg) + " V" for vg in vgs_exp], offsets=offsets_exp)
# Plot Fits
rvt_obj.global_RTVg_plot(function=graphene.Graphene_Phonons.rho_Graphene_on_SiO2, params=tuple(params_exp), ax=exp_ax, c=colourCycle2, linewidth=0.3, singleLabel="Ave fit SiO2", offsets=offsets_exp)
rvt_obj.global_RTVg_plot(function=graphene.Graphene_Phonons.rho_Graphene_on_Dielectric, params=tuple(params_exp_e0), ax=exp_ax, c=colourCycle, points=30, linewidth=0.3, style="-.", singleLabel="Ave Fit (E0=%0.02f meV)" % (params_exp_e0[3]*1000), offsets=offsets_exp)
# LEGEND
...
param_rows = ["Ave SiO2", "Ave Dielectric"]
param_list = []
...
plt.savefig(target2 + "02 Phonons %0.02f" % temps_exp[0] + "K-%0.02f" % temps_exp[-1] + "K.png", bbox_inches="tight")
